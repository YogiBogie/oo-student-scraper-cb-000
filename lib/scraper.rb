...
class Scraper
  ...
  def self.scrape_index_page(index_url)
    ...
    student.each do |info|
      profile = {:name => info.css(".student-name").text,
        :location => info.css(".student-location").text,
        :profile_url => info.css("a").attribute("href").text
      }
      students << profile
    end
    return students
  ...
  def self.scrape_profile_page(profile_url)
    ...
    profile.css(".social-icon-container a").each do |website|
    ...
    #Get profile quote
    hash[:profile_quote] = profile.css(".vitals-text-container div.profile-quote").text
    #Get profile bio
